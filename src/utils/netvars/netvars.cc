...
namespace netvars {
	void init() {
		for (auto data = interfaces::m_client->get_all_classes(); data; data = data->m_next) {
	...
	void dump_recursive(const char* base_class, recv_table_t* table, uint32_t offset) {
		for (int i = 0; i < table->m_props_count; i++) {
			...
			if (!prop || isdigit(prop->m_var_name[0]) || !strcmp(prop->m_var_name, _("baseclass")))
				continue;

			if (prop->m_data_table && prop->m_recv_type == DPT_DATATABLE && prop->m_data_table->m_net_table_name[0] == 'D')
				dump_recursive(base_class, prop->m_data_table, offset + prop->m_offset);

			char name[256];

			strcpy(name, base_class);
			strcat(name, _("->"));
			strcat(name, prop->m_var_name);
